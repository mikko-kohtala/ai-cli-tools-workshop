...
const sessionGuidelines = [
  ...
  },
  {
    ...
    description:
      "For complex tasks, ask the agent to create a plan first. Review it, suggest changes, then execute to prevent misunderstandings.",
    example:
      "You: 'Create a plan for adding dark mode support'\nAgent: [shows 5-step plan]\nYou: 'Skip step 3, we already have the theme context'\nAgent: 'Updated plan: ...'",
  },
  {
    title: "Summarize changes with file references and command suggestions",
    description:
      "After the agent makes changes, ask for a summary. This helps you understand what happened and makes it easier to review.",
    example:
      "You: 'What files did you change?'\nAgent: 'Modified src/api/client.ts (added retry logic) and tests/api.test.ts (added retry tests)'",
  },
  {
    title: "Offer next steps like tests or commits without overstepping",
    description:
      "Agents can suggest next steps, but you're always in control. They might suggest running tests or committing changes, but won't do it without permission.",
    example: "Agent: 'Changes complete. Would you like me to: 1) Run the tests 2) Show a diff 3) Create a git commit?'",
  },
];
  ...
  return (
    <section className="mb-16 scroll-mt-16" id="sessions">
      <div className="mb-6 flex items-center gap-3">
        ...
        <h2 className="font-bold text-4xl">Effective Sessions</h2>
      </div>
